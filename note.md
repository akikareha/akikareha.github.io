...
組み込みの各カラースキームのスクリーンショットの一覧を作ると世の中の役に立つかもしれない。
高々25個。
されど25個。
思い付いたからには、やらないといけないのか。
`micro-screenshots` などとして、野良カラースキームも含めてスクリーンショットを載せると、結構良いかもしれない。
作りはこのサイトと同じような感じで大丈夫だろう。
...
## microのオプションをトグルするプラグイン

microのオプションをトグルするコマンドがmicro本家のイシュに投稿されてた。
今はそれを `init.lua` に書いて使ってるけど、独立したプラグインに整えたい。
完全にパクリになるが。
...
[GitHubページ](github-pages.html)のページに加筆しないと。
